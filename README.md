...
- 쿠폰/포인트 모달 기능 구현(1) : 슈퍼베이스 DB를 기반으로 쿠폰/포인트 보유 수 데이터 바인딩 후 사용할 포인트 인풋창에 숫자만 입력 가능한 정규식 적용
- 쿠폰/포인트 모달 기능 구현(2) : 전체사용 버튼 클릭시 state값에 보유한 포인트 state값을 대입 후 input value에 해당 스테이트 값을 데이터 바인딩 
- 쿠폰/포인트 모달 기능 구현(3) : if문을 사용하여 보유 포인트에 비례하여 “적용” 버튼 클릭 시 간단한 alert 처리
