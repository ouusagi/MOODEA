...
## 更新履歴
### 2025-12-24
- 会員情報修正ロジック（1）: ニックネームの重複チェックを行う際、usersテーブルを直接参照するとセキュリティ上の問題があるため、
usersテーブルを参照する View を作成し、id と username のみ取得可能にしました。
その View を利用して、ニックネームの重複チェックロジックを実装しました。
- 会員情報修正ロジック（2） : すべての input 要素に値を入力した後、「情報修正」ボタンをクリックすると、
入力された値を UPDATE する仕組みを実装しました。
...
## カスタマーサポートページ
- カスタマーサポートページ ：カスタマーサポートページのUIを実装
- カスタマーサポートページ（アコーディオン）：質問タブクリック時にクラスを付与し、アコーディオンの開閉を実装
- カスタマーサポートページ（検索機能）：検索結果を state に保持し、includes() による判定結果に応じてレンダリングを制御
...
- 決済ポイント：購入時に付与予定のポイントを OrderHeaders テーブルに一時保存し、決済完了後、サーバーに保存されたポイント値を基準にユーザーポイントを更新
- 決済クーポンの使用：購入時に選択したクーポンIDを OrderHeaders テーブルに一時保存し、決済完了後に該当クーポンを user_coupons テーブルで used = true に更新する。
- ポイント利用処理：購入時に使用予定ポイントを OrderHeaders テーブルに一時保存し、決済完了後に [既存の保有ポイント + 獲得ポイント − 使用ポイント] を計算して users テーブルのポイントを更新
